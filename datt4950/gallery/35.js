author = "Rose Zhou"
title = "Strange Attractor - Mutate"

var pop = [];
var dim = 50;
var rows = 10;
var gene_size = 12; // a set corresponds to fractal coefficients

// probability that an individual will make an approximate copy of itself at reproduction
var close_copy_probablity = 0.05;
...
// find corresponding coefficient from letter
...
function draw_pts(individual) {
  ...
  // iterate many times each time
  for (var i = 0; i < 12; i++) {
    var x = individual.pos[0];
    var y = individual.pos[1];
    // console.log(x, y);
    individual.img.deposit(1, x/4 + dim/4, y/4 + dim/2);
    // update next point
...
is_auto && setInterval(check_reproduce, 150);
...
function draw() {
  ...
  for (var i = 0; i < pop.length; i++) {
    ...
    if(is_mono==false){
      draw2D.hsl(1 - 1 / pop.length * i, 0.7, 0.4, 0.6);
    }else{
      draw2D.color("white");
    }
    draw_pts(pop[i]);
