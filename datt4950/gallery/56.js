/*
Changed the field.get(x,y) in odd-index and even-index.
NE value x-1 == prepetually white field
NE == NW = a more subtle version with concentrated neighbors

As the transition of left to right happens, those indidivual small ones move up and down untill it is either carried away in the ways or form larger groups.

If dimensions of grid increases, a noticiable defect is present.
There is a gap area around the top that uniquely becomes a 'dead zone' in a sense but the single streak of artificial life stagnated is a let down.
...
function reset() {
  ...
  field.set(function(x, y) {
    return (x == 128 || y == 128 || random(1000) == 0) ? 1 : 0;
  })
}
reset();
...
//  boundaryvalue = 0;
boundaryvalue = function() { return random(2); }
...
function update() {
    ...
    for (var y = 2; y < field.height; y += 1) {
      for (var x = 1; x < field.width; x += 2) {
        var NW = field.get(x, y)
       // var NE = field.get(x -1, y)
        
        var NE = field.get(x , y)
        var SE = field.get(x -1, y + 1)
        var SW = field.get(x, y + 1)
