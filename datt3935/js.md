...
Throw exceptions with ```throw <exception-object>;```, and catch with the ```try...catch``` control flow. A throw will immediately exit the current flow. The exception-object can be anything, but typically an object with a ```.message``` property for the ```catch``` to deal with. 
